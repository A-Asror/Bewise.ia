...
# from backend.models.db.account import Account
# from backend.models.schemas.jwt import  JWTAccount, JWToken
# from backend.utils.exceptions.database import EntityDoesNotExist
#
#
# class JWTGenerator:
#
...
#         else:
#             expire = dt.datetime.utcnow() + dt.timedelta(minutes=settings.JWT_MIN)
#
#         to_encode.update(JWToken(exp=expire, sub=settings.JWT_SUBJECT))
#
...
#             expires_delta=dt.timedelta(minutes=settings.JWT_ACCESS_TOKEN_EXPIRATION_TIME),
#         )
#
#     def retrieve_details_from_token(self, token: str, secret_key: str) -> list[str]:
#         try:
#             payload: dict = jose_jwt.decode(token=token, key=secret_key, algorithms=[settings.JWT_ALGORITHM])
